...
void Model::set_material(uint32_t id, aiMaterial const& material) noexcept {
    ...
    m.emission_texture = aiTextureType_to_string(material, aiTextureType_EMISSION_COLOR);
    if (m.emission_texture.empty()) {
        m.emission_texture = aiTextureType_to_string(material, aiTextureType_EMISSIVE);
    }
